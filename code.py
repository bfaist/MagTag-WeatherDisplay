...
def get_forecast(location):
    ...
    json_data = resp.json()
    return json_data["current"], json_data["daily"], json_data["current"]["dt"], json_data["timezone_offset"]
...
def update_today(data, temp_data, tz_offset=0):
    ...
    today_morn_temp.text = temperature_text(temp_data["morn"])
    today_day_temp.text = temperature_text(temp_data["day"])
    today_night_temp.text = temperature_text(temp_data["night"])
...
def refreshWeather():
    ...
    today_weather, forecast_data, utc_time, local_tz_offset = get_forecast(latlon)

    print("Updating...")
    update_today(today_weather, forecast_data[0]["temp"], local_tz_offset)
...
while True:
    currentButtonState = magtag.peripherals.button_a_pressed
    # print("Current Button State : " + str(currentButtonState) + " : " + str(previousButtonState))

    if currentButtonState and not previousButtonState:
        # print("Button A pressed")
        refreshWeather()
    previousButtonState = currentButtonState
    time.sleep(magtag.display.time_to_refresh + 3)
